...
## ⚠️ CRITICAL: Frontend Location (MUST READ FIRST)

**THE OFFICIAL FRONTEND IS: `universal-app/`**

All frontend development work MUST happen in `/universal-app/` directory. This is the actively developed, production-targeted frontend.

**DO NOT work on:**
- `minute-main/frontend/` - ❌ Legacy/outdated, not for new development
- `apps/web/` - ❌ Prototype, not production
- Any other frontend directory

**Before any frontend work:**
1. Verify your working directory is `universal-app/`
...
4. Frontend API calls must flow through `universal-app/src/lib/api-client.ts` and the generated client in `universal-app/src/lib/api/generated`; regenerate with `pnpm --filter universal-app openapi-ts` after OpenAPI contract changes instead of hand-editing types.
...
13. Offline/PWA state and retries belong in shared utilities (e.g., `universal-app/src/lib/offline-queue.ts`); avoid ad-hoc storage logic.
...
18. ** mobile shell**: RN/Expo code lives under `apps/mobile/`; changes there must not modify web files. Shared web UI primitives belong in `universal-app/src/components/ui/`; add demos under `universal-app` routes such as `/ui-demo` rather than ad-hoc pages.
...
31. Update the root `CHANGELOG.md` for every code, task, script, or documentation change; `universal-app/CHANGELOG.md` is archival only. Never skip this even for trivial edits.
...
33. After adopting Turbopack or typed routes, regenerate the OpenAPI client (`pnpm --filter universal-app openapi-ts`), run `next build --turbopack`, and re-check internal links/navigation with Playwright smoke tests before shipping.
...
35. For Next.js 16 / React 19 app-router layouts, type props with `ReactNode` (not `ReactElement`), keep route params unwrapped (no Promise), and resolve Radix/msal typings via upgrades or explicit overrides before shipping.
...
37. Treat `minute-main/backend` and `minute-main/worker` as the primary social-care backend bounded context, and treat `universal-app/` as the canonical web shell that must not diverge architecturally from `minute-main/docs/*` without updating those docs.
...
48. Frontend build/run: install from repo root with `pnpm install`, run `pnpm --filter universal-app dev` for UI work (or `pnpm --filter universal-app build`), and avoid running `npm install` inside sub-packages or treating `minute-main/frontend` as an active workspace; helper scripts `scripts/setup-frontend.sh` and `scripts/dev-frontend.sh` are the canonical entry points.

## 12) universal-app Feature Inventory (Production-Ready)

The official frontend (`universal-app/`) includes the following production-ready features as of 2026-03-28:

### Core Infrastructure
- **API Client**: Typed HTTP client with retry logic, interceptors, auth token management (`src/lib/api-client.ts`)
- **Error Handling**: ApiError, NetworkError, TimeoutError classes with ErrorBoundary (`src/lib/api-errors.ts`)
- **OpenAPI Client**: Generated TypeScript client from backend spec (`openapi-ts.config.ts`, `src/lib/api/`)
- **TanStack Query**: Data fetching hooks with caching (`src/lib/useApiQuery.ts`)
- **Backend Proxy**: API proxy route for backend communication (`src/app/api/proxy/`)

### Authentication & Authorization
- **MSAL Integration**: Azure Entra ID authentication (`src/lib/auth/`, `src/providers/AuthProvider.tsx`)
- **Auth Hooks**: useAuth(), useAccessToken() (`src/hooks/useAuth.ts`)
- **Role-Based Access**: Permission system with role checks
- **Demo Mode**: Development bypass with mock data

### Offline & PWA
- **Offline Queue**: Dexie IndexedDB storage with retry (`src/lib/offline-queue.ts`, `src/lib/storage-adapter.ts`)
- **Service Worker**: Caching and background sync (`public/sw.js`)
- **PWA Manifest**: Full manifest with icons (`public/manifest.json`, `src/app/manifest.ts`)
- **Connectivity Indicator**: Network status monitoring (`src/components/ConnectivityIndicator.tsx`)
- **Resilience Banner**: Offline status display (`src/components/ResilienceBanner.tsx`)

### Multi-Tenant System
- **Tenant Configuration**: TenantConfig, ServiceDomain, RoleConfig (`src/lib/tenant/`)
- **Module Registry**: Dynamic feature loading (`src/lib/modules/`)
- **Feature Flags**: PostHog-integrated toggles (`src/lib/features/`)
- **Domain Switching**: Service domain switching UI (`src/lib/domain/`, `src/components/domain/`)

### Theme & Styling
- **Theme Engine**: OKLCH colors with CSS variables (`src/lib/themes/`)
- **Dark Mode**: System preference + manual toggle
- **Glassmorphism**: Premium glass effects
- **Council Branding**: WCC, RBKC themes

### Recording & Capture
- **Audio Recorder**: MediaRecorder with visualizer (`src/lib/audio/`)
- **Recording Page**: Full capture flow (`src/app/record/`)
- **Quality Settings**: Multiple bitrate options
- **Case Metadata**: Meeting context attachment

### Transcription & Minutes
- **Transcript Viewer**: Timestamped segments, speaker labels (`src/components/transcription/`)
- **Minute Editor**: Section-based editing (`src/components/minutes/`)
- **Evidence Linking**: Click-to-seek timestamps
- **Template System**: Pre-configured templates (`src/lib/templates/`, `src/components/templates/`)

### Review & Approval
- **Review Queue**: Filterable manager queue (`src/components/review/`)
- **Workflow System**: State machine with transitions (`src/lib/workflow/`)
- **Approval Dialogs**: Approve/reject/request changes
...
### Document Export
- **Export System**: Word, PDF, TXT, HTML formats (`src/lib/export/`)
- **SharePoint Integration**: Microsoft Graph API (`src/lib/sharepoint/`)
- **Council Templates**: Branded export templates

### Notifications
- **Push Notifications**: Service worker push (`src/lib/notifications/push.ts`)
- **Notification Center**: In-app notifications (`src/components/notifications/`)
- **Preferences**: Category toggles, quiet hours

### Accessibility
- **WCAG AA**: Contrast, focus, keyboard (`src/lib/a11y/`)
- **Skip Links**: Screen reader navigation
- **Live Regions**: Dynamic announcements
- **Focus Trapping**: Modal accessibility

### Mobile & Responsive
- **Breakpoint Hooks**: useIsMobile(), useBreakpoint() (`src/hooks/useResponsive.ts`)
- **Mobile Components**: BottomSheet, SwipeActions, FAB (`src/components/mobile/`)
- **Touch Gestures**: Swipeable views

### Performance
- **Loading Skeletons**: All content types (`src/components/skeletons/`)
- **Optimistic Updates**: Instant UI feedback (`src/lib/optimistic/`)
- **Bundle Optimization**: Code splitting, lazy loading (`src/lib/lazy/`)
- **Performance Budgets**: Lighthouse CI configuration

### Testing & CI/CD
- **Unit Tests**: Vitest setup (`vitest.config.ts`)
...
- **PostHog**: Analytics with feature flags (`src/lib/posthog.ts`)
- **Custom Events**: Unified analytics hook (`src/hooks/useAnalytics.ts`)
