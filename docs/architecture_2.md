...
## 1. Executive Summary
- **Goal:** Build a universal, configuration‑driven council app (web + mobile) that supports multiple councils, departments, and roles from a single codebase, without spawning sub‑apps – with the existing Minute social‑care product acting as the first feature pack running inside this universal shell.
...
- **Success criteria:**
  ...
  - Persona journeys (social worker, manager, admin, QA, digital) are captured in `minute-main/docs/user_journeys.md` and directly inform shell layout, navigation, and module design.
...
### 11.1 Universal app phases (monorepo)
...
### 11.2 Crosswalk to Minute roadmap phases

The social‑care roadmap in `minute-main/ROADMAP_social_care.md` reuses these ideas and extends them:

- Phases 16–21: config schema, module registry, multilingual support, and tasks map onto tenant config, plugins, and DAL layers described here.
- Phases 22–26: mobile maturity, premium app shell, and adaptive UX mirror the RN/RN‑Web shells and plugin‑driven navigation defined in §§4 and 6.
- Phases 27–29: Recording Studio 2.0, AI writing assistant, and contextual tabs bring the Magic Notes–style capture and editing experience onto the universal shell while staying config‑driven.
- Phases 30–36: universal config system, design tokens, advanced offline/sync, admin console, and telemetry/adoption dashboards harden this architecture for additional departments and councils.
- Phases 37–40: collaboration, search, compliance tools, and performance optimisation sit on top of the plugin/DAL system, without changing the core tenancy model.

When adding or adjusting phases in either roadmap, keep this crosswalk in sync so we avoid diverging “Minute‑only” and “Universal app” plans.
