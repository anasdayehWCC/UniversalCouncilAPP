...
export default function ReviewQueueError({
  ...
  React.useEffect(() => {
    console.error('[review-queue/error.tsx] Error caught:', error);
  }, [error]);
    ...
    <ShellPage className="flex items-center justify-center bg-background transition-colors" padded={false} contentClassName="flex h-full items-center justify-center p-6">
      <Card className="max-w-lg w-full shadow-lg border-destructive/20">
        <CardHeader className="text-center pb-4">
          ...
          <CardTitle className="text-xl">Review Queue Unavailable</CardTitle>
          <CardDescription className="text-base">
            The review queue couldn&apos;t be loaded. This may be a temporary issue.
          </CardDescription>
          {error.digest && (
